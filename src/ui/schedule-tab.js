...
export function renderSchedule() {
  ...
  for (let i = 0; i < state.schedule.length; i++) {
    ...
    html +=
      ...
      '" data-sched-idx="' +
      i +
      '" data-sched-field="prefix" placeholder="Optional liturgical note"></div>' +
      '<div style="display:flex;gap:3px">' +
      '<button class="btn-sm" data-sched-move="' +
      i +
      '" data-dir="-1">\u2191</button>' +
      '<button class="btn-sm" data-sched-move="' +
      i +
      '" data-dir="1">\u2193</button>' +
      '<button class="btn-sm del" data-sched-del="' +
      i +
      '">\u2715</button>' +
      '</div></div></div>';
  }
